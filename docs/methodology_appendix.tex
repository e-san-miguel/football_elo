...
%----------------------------------------------------------------------
\section{Experimental: Squad-Strength Adjustment}
\label{sec:squadstrength}
%----------------------------------------------------------------------

\subsection{Motivation}

National teams play only 8--12 matches per year, so Elo ratings can lag meaningful roster changes. A team whose squad has dramatically improved may carry an outdated rating, and aging superstars whose on-field contribution remains high are not distinguished from declining players by Elo alone. This section documents an experiment to inject player-level squad-strength information via Transfermarkt (TM) market values, with calibration against historical World Cups.

\paragraph{Status.} The calibrated weight does not clear the decision gate described in Section~\ref{sec:squadcalibration}. The published predictions continue to use pure Elo. This section is preserved for methodological transparency and to support future iteration.
...
\subsection{Composite Rating and Calibration}
...
\paragraph{Calibration objective.} We grid-search $\beta \in \{0.0, 0.1, \ldots, 1.5\}$ on the pooled match outcomes of the 2018 and 2022 men's World Cups, minimizing the mean multiclass Brier score:
\[
\mathcal{B}(p, y) = \sum_{k \in \{W, D, L\}} \bigl(p_k - \mathbb{1}\{y = k\}\bigr)^2
\]
applied to every match in the two tournaments.

\paragraph{Cross-validation.} $\beta$ is fit on one tournament and evaluated on the other, reporting out-of-sample Brier lift.

\paragraph{Decision gate.} A 5\% out-of-sample Brier improvement over pure Elo is required before the composite is deployed to published predictions.
...
The grid search selects $\beta^\star = 0.2$ with pooled Brier 0.5924, versus 0.5954 for pure Elo --- a 0.50\% relative lift in sample, still below the 5\% decision gate. Cross-validated lifts are asymmetric: $+0.63\%$ when training on 2022 and evaluating on 2018, but $-0.20\%$ when training on 2018 and evaluating on 2022 ($\beta = 0.3$ appears overfit to 2018-specific patterns). A more conservative $\beta = 0.1$ fits both tournaments cleanly but gives a smaller in-sample lift.

\paragraph{Diagnosis.} The log-transformed squad score $z_T$ has Pearson correlation of $0.79$ (2018) and $0.78$ (2022) with normalized Elo --- nearly identical to the correlation without the log transform. Elo already captures the bulk of squad-quality information: strong teams accumulate high Elo because they have good rosters. The log transform improves the functional form but does not reveal new signal; the residual squad information orthogonal to Elo remains small and noisy over the 128-match backtest window.

\paragraph{Statistical power.} With per-match Brier standard deviation $\approx 0.3$ and $n = 128$ matches, the standard error on the mean is $\approx 0.027$. A true 5\% lift ($\approx 0.030$ Brier points) would be detectable at roughly 50\% power; a 2\% lift ($\approx 0.012$ points) requires on the order of 1{,}100 matches. The backtest can rule out large effects but is underpowered for small ones.

\subsection{Illustrative 2026 Output}

Applying the composite rating at the conservative $\beta = 0.1$ setting to current TM squads (players active in the 2023/24 season or later, top 26 by market value per team) yields the tournament-level shifts shown in Table~\ref{tab:composite2026}. Top European squads gain modestly while smaller-squad nations shift down; Mexico loses ground relative to Portugal, reflecting that Elo was overvaluing Mexico's host-boosted rating.

\begin{table}[h]
\centering
\begin{tabular}{@{}lrrrr@{}}
\toprule
Team & Elo & Composite & Pure $P(\text{Win})$ & Composite $P(\text{Win})$ \\
\midrule
Spain     & 2218 & 2245 & 32.0\% & 33.3\% \\
Argentina & 2179 & 2199 & 19.8\% & 20.4\% \\
France    & 2137 & 2165 & 14.6\% & 16.1\% \\
England   & 2084 & 2112 & 6.4\%  & 7.1\%  \\
Brazil    & 2045 & 2070 & 3.5\%  & 3.6\%  \\
Portugal  & 2035 & 2057 & 2.7\%  & 2.8\%  \\
Mexico    & 1960 & 1961 & 2.1\%  & 1.4\%  \\
\bottomrule
\end{tabular}
\caption{Illustrative 2026 WC win probabilities: pure Elo vs composite ($\beta = 0.1$, log-transformed squad values). These outputs are not currently used on the live site.}
