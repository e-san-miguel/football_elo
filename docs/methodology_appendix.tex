...
\label{eq:gfactor}
\end{equation}
where $g = \text{home\_score} - \text{away\_score}$ is the goal difference.

\paragraph{Remark.} The original eloratings.net system uses $G = (11 + |g|) / 8$ for $|g| \geq 3$, which grows without bound. We cap $G$ at 1.75 to prevent disproportionate rating inflation from lopsided results against weak opponents. A 3-0 victory is treated as equally convincing as an 8-0 victory.
...
\subsection{Group Stage Match Probabilities}

For group stage matches, we decompose the Elo expected score $W_e$ into separate win, draw, and loss probabilities.

\paragraph{Draw probability model.} The probability of a draw is modeled as a function of the expected score:
\begin{equation}
P(\text{draw})_{\text{raw}} = 0.38 - \frac{0.38 \cdot (W_e - 0.5)^2}{0.25}
\label{eq:pdraw}
\end{equation}
\begin{equation}
P(\text{draw}) = \min\!\big(\max(P(\text{draw})_{\text{raw}},\, 0),\, 0.38\big)
\end{equation}

This parabolic model achieves a maximum draw probability of 38\% when teams are equally rated ($W_e = 0.5$) and decreases as the rating gap widens, reaching zero when $|W_e - 0.5| \geq 0.5$.

\paragraph{Win/loss probabilities.} Given the draw probability:
\begin{align}
P(\text{win}_A)_{\text{raw}} &= W_e - \tfrac{1}{2}\, P(\text{draw}) \\
P(\text{win}_B)_{\text{raw}} &= (1 - W_e) - \tfrac{1}{2}\, P(\text{draw})
\end{align}
Both values are clamped to $[0, \infty)$ and then normalized so that $P(\text{win}_A) + P(\text{draw}) + P(\text{win}_B) = 1$.

\paragraph{Home advantage.} Host nations (United States, Mexico, Canada) receive the standard $H = 50$ home advantage when playing in their own country, applied to both group stage and knockout matches.

\subsection{Group Stage Simulation}

For each of the 12 groups, the six pairwise matches are simulated independently. Match outcomes are sampled from the categorical distribution $(P(\text{win}_A),\, P(\text{draw}),\, P(\text{win}_B))$. Points are awarded as 3 for a win, 1 for a draw, and 0 for a loss.

Goal difference is approximated for tiebreaking purposes: a winning team receives $+1.5$ goal difference and the losing team $-1.5$; draws contribute $0$ to both teams.
...
Knockout matches (R32 through the Final) are simulated without draws. The win probability is the Elo expected result:
\begin{equation}
P(\text{win}_A) = W_e = \frac{1}{10^{-(R_A + h - R_B)/400} + 1}
\end{equation}
